...
namespace ExcelDataReader.Portable
{
	...
	public class ExcelOpenXmlReader : IExcelDataReader
	{
		...
		private async Task ReadGlobalsAsync()
		{
			...
			using (var stream = await _zipWorker.GetWorkbookStream()) {
				sheets = ReadWorkbook(stream);
			}
			using (var stream = await _zipWorker.GetWorkbookRelsStream()) {
				ReadWorkbookRels(stream, sheets);
			}

			using (var stream = await _zipWorker.GetSharedStringsStream()) {
				sst = ReadSharedStrings(stream);
			}

			using (var stream = await _zipWorker.GetStylesStream()) {
